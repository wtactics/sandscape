 ...
 * elFinder.php
 * 
 * This is integrated into StaySimple, it wasn't created by StaySimple's developers.
 ...
 * 
 * Please look at elFinder website for more information:
...
class elFinder {
    ...
    private $_options = array(
        ...
        'tmbSize' => 48,
        'fileURL' => true,
        'dateFormat' => 'j M Y H:i', //TODO: ... use settings?
        'logger' => null,
        'aclObj' => null,
        'aclRole' => 'user',
        'defaults' => array(
    ...
    private function _dirSize($path) {
        ...
        if (!isset($this->_options['du'])) {
            $this->_options['du'] = function_exists('exec') ? exec('du -h ' . escapeshellarg(__FILE__), $o, $s) > 0 && $s == 0 : false;
        }
        if ($this->_options['du']) {
    ...
    private function _tmbPath($path) {
        $tmb = '';
        if ($this->_options['tmbDir']) {
            $tmb = dirname($path) != $this->_options['tmbDir'] ? $this->_options['tmbDir'] . DIRECTORY_SEPARATOR . $this->_hash($path) . '.png' : $path;
        }
        return $tmb;
    }
